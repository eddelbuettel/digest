...
for (alg in c("sha1", "md5", "crc32", "sha256", "sha512",
    ...
    expect_true(identical(h1,h2), info = alg)
    h3 <- digest(substr(x,1,18000)  , algo=alg, serialize=FALSE)
    expect_true(identical(h1,h3), info = alg)
    #cat(h1, "\n", h2, "\n", h3, "\n")
    expect_identical(
    ...
    expect_true( identical(h4,h5), info = alg )
    ...
    expect_true( identical(h6, h7), info = alg )
    expect_identical(
...
expect_true( identical(h1,h2), info = "md5 vs tools::md5sum" )
